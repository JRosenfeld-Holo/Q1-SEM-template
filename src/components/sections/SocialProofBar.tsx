...
const LOGOS: { name: string; src: string; renderH: number; renderW: number }[] = [
  ...
  { name: "Sunday Power", src: "/customer-logos/sunday-power.avif", renderH: 28, renderW: 155 }, // 7728/1394 × 28
...
function LogoList({ ariaHidden = false, priority = false }: { ariaHidden?: boolean; priority?: boolean }) {
  return (
    <ul
    ...
    >
      {LOGOS.map((logo, i) => (
        <li key={`${logo.name}-${i}`} className="flex items-center">
          <span
            className="px-10 md:px-14 transition-all duration-300 flex items-center"
            style={{
              opacity: "var(--theme-logo-opacity)" as unknown as number,
            }}
          >
            <Image
              src={logo.src}
          ...
          <span className="text-xs" style={{ color: "var(--theme-border)" }} aria-hidden="true">·</span>
        </li>
      ))}
    </ul>
  );
    ...
    >
      ...
      <div className="relative overflow-hidden">
        ...
        <div
          ...
          style={{ background: `linear-gradient(to right, var(--theme-fade-from), transparent)` }}
          aria-hidden="true"
        />
        <div
          className="absolute right-0 top-0 bottom-0 w-20 z-10 pointer-events-none"
          style={{ background: `linear-gradient(to left, var(--theme-fade-from), transparent)` }}
          aria-hidden="true"
        />
