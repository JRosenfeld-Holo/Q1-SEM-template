...
const MAX_SELECTIONS = 7; // 6 continents + Middle East
...
const getRegionName = (lat: number, lon: number): string => {
  ...
  // Middle East before Africa to avoid overlap (lat 10-42, lon 32-65)
  ...
  // Oceania before Asia (catches Pacific islands)
...
// Shaders defined outside component to avoid string recreation on each render
const VERTEX_SHADER = `
  varying vec2 vUv;
  varying vec3 vPos;
  void main() {
...
// Land mask — returns false when data not yet loaded (prevents premature ocean clicks)
class LandTester {
  ...
  isLand(uv: THREE.Vector2 | undefined): boolean {
    // Return false (not land) when not loaded — prevents clicks before mask is ready
...
// Shared texture promise — prevents double network request
...
// Country border lines decoded from TopoJSON (world-atlas)
const CountryBorders = memo(() => {
    ...
    (async () => {
      try {
        ...
        for (const arc of topo.arcs as number[][][]) {
          // TopoJSON: first point is absolute, rest are deltas
          let ax = 0, ay = 0;
          const decoded: [number, number][] = arc.map(([dx, dy]: [number, number]) => {
            ax += dx;
            ay += dy;
            return [ax * scale[0] + translate[0], ay * scale[1] + translate[1]];
          });

          // Build line segments (pairs of points) from the arc strip
  ...
  // Clean up geometry/material on unmount or re-creation
  useEffect(() => {
    return () => {
      if (lineSegments) {
...
const RingAnim = memo(({ scale, speed, delay = 0, color }: { scale: number, speed: number, delay?: number, color: string }) => {
...
// Marker position snaps to continent centroid (ContinentConfig), not raw click coords
const SelectionMarker = memo(({ lat, lon }: { lat: number; lon: number }) => {
...
const Globe = memo(({ onSelect, selectedRegions, onLoaded }: {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < MAX_SELECTIONS; i++) {
      ...
      if (region) {
        ...
        const [x, y, z] = latLonToArray(
          config ? config.lat : region.lat,
          config ? config.lon : region.lon,
          1.51
        );
        points[i].set(x, y, z);
  ...
  return (
    <group rotation={[0, 0, 0.15]} dispose={null}>
      ...
      <Sphere
        ...
        onPointerMove={(e) => {
          e.stopPropagation();
          const land = landTester.isLand(e.uv);
          document.body.style.cursor = land ? 'pointer' : 'auto';
        }}
        onPointerOut={() => {
        ...
        onClick={(e) => {
          ...
          if (landTester.isLand(e.uv)) {
            ...
            if (!isNaN(lat) && !isNaN(lon)) {
              onSelect({ name: getRegionName(lat, lon), lat, lon });
            }
          }
        }}
      >
      ...
      {/* Country borders rendered just above the shader sphere */}
      <CountryBorders />
      ...
      {/* Markers snap to ContinentConfig centroid, not raw click position */}
      {selectedRegions.map((region) => {
        const config = ContinentConfig[region.name];
        return (
          <SelectionMarker
            key={region.name}
            lat={config?.lat ?? region.lat}
            lon={config?.lon ?? region.lon}
          />
        );
      })}
    </group>
...
// Controls as a sub-component so isDragging state doesn't cause Canvas re-mounts
const Controls = memo(({ rotationSpeed }: { rotationSpeed: number }) => {
